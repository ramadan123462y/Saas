        ...
        <div class="max-w-full md:max-w-6xl mx-auto my-3 md:px-8">
            <div class="relative block flex flex-col md:flex-row items-center">
                    ...
                    <div class="bg-white text-black rounded-lg shadow-inner shadow-lg overflow-hidden">
                        <div class="block text-left text-sm sm:text-md max-w-sm mx-auto mt-2 text-black px-8 lg:px-6">
                            <h1 class="text-lg font-medium uppercase p-3 pb-0 text-center tracking-wide">
                                {{-- {{ $plans[0]->templete->num_templete }} --}}
                            </h1>

                            <h2 class="text-sm text-gray-500 text-center pb-6">€{{ $plans[0]->price }} /mo</h2>
                        ...
                        <div class="block flex items-center p-8  uppercase">
                            @php
                                $plan_id = $plans[0]->id;
                            @endphp
                            <form action='{{ url("saller/go_to_pay/$store_id/$plan_id") }}' style="width: 100%"
                                method="POST">
                                @csrf
                                <input type="submit" name="type" value="Pay With Card"
                                    class="mt-3 text-lg font-semibold  bg-black w-full text-white rounded-lg px-6 py-3 block shadow-xl hover:bg-gray-700">


                                <input type="submit" name="type" value="Pay With Wallet"
                                    class="mt-3 text-lg font-semibold  bg-black w-full text-white rounded-lg px-6 py-3 block shadow-xl hover:bg-gray-700">
                            </form>
                        </div>
                    </div>
                ...
                <div class="w-full max-w-md sm:w-2/3 lg:w-1/3 sm:my-5 my-8 relative z-10 bg-white rounded-lg shadow-lg">
                    ...
                    <div class="block text-left text-sm sm:text-md max-w-sm mx-auto mt-2 text-black px-8 lg:px-6">
                        <h1 class="text-lg font-medium uppercase p-3 pb-0 text-center tracking-wide">
                            {{-- {{ $plans[1]->templete->num_templete }} --}}
                        </h1>
                        <h2 class="text-sm text-gray-500 text-center pb-6"><span
                                class="text-3xl">€{{ $plans[1]->price }}</span> /mo
                        </h2>
                    ...
                    <div class="block flex items-center p-8  uppercase">
                        @php
                            $plan_id = $plans[1]->id;
                        @endphp
                        <form action='{{ url("saller/go_to_pay/$store_id/$plan_id") }}' style="width: 100%" method="POST">
                            @csrf
                            <input type="submit" name="type" value="Pay With Card"
                                class="mt-3 text-lg font-semibold  bg-black w-full text-white rounded-lg px-6 py-3 block shadow-xl hover:bg-gray-700">
                ...
                <div
                    ...
                    <div class="bg-white text-black rounded-lg shadow-inner shadow-lg overflow-hidden">
                        <div class="block text-left text-sm sm:text-md max-w-sm mx-auto mt-2 text-black px-8 lg:px-6">
                            <h1 class="text-lg font-medium uppercase p-3 pb-0 text-center tracking-wide">
                                {{-- {{ $plans[2]->templete->num_templete }} --}}
                            </h1>
                            <h2 class="text-sm text-gray-500 text-center pb-6">€{{ $plans[2]->price }} /mo</h2>
                        ...
                        <div class="block flex items-center p-8  uppercase">
                            @php
                                $plan_id = $plans[2]->id;
                            @endphp
                            <form action='{{ url("saller/go_to_pay/$store_id/$plan_id") }}'
...
<footer class="pt-4">
    <a href="https://twitter.com/framansi" target="_blank">
        <div class="flex flex-row items-center justify-center">
            ...
            <svg class="h-5 px-2 cursor-pointer fill-current text-blue-200 hover:text-blue-300" viewBox="0 0 30 24"
