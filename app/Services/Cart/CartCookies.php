...
class CartCookies implements CartInterface
{
    ...
    public function cart()
    {
        ...
        $cart = Cart::with('cart_items')->Cookie()->first();
        $cartitems = [];
        if (isset($cart)) {
    ...
    public function updateorcreate($request, $id_product)
    {
        ...
        $cart = Cart::where('cookie_id', $cart_id)->first();
    ...
    public function generate_cart_id()
    {
        ...
        $cart_id = Cookie::get('card_id');
        if (isset($cart_id)) {

            $cart_id = $cart_id;
        } else {

            Cookie::queue('card_id', Str::uuid(), 60 * 60 * 60 * 24, null);
            $cart_id = Cookie::get('card_id');
        }
        return $cart_id;
    }
